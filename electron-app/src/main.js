...
function getBackendPath() {
  ...
  if (isPackaged) {
    ...
    console.log(
      `Packaged app detected. Resources path: ${process.resourcesPath}`
    );
  } else {
    ...
    if (os.platform() === "win32") {
      ...
      exec(
        ...
        (error) => {
          if (error) {
            console.log(
              `No process found on port ${port} or failed to kill: ${error.message}`
            );
          } else {
...
async function startBackend() {
  ...
  if (!fs.existsSync(keyFilePath)) {
    ...
    if (fs.existsSync(homeKeyPath)) {
      console.log(
        `Found encryption key at ${homeKeyPath}, copying to ${keyFilePath}`
      );
      try {
  ...
  try {
    ...
    console.warn(
      `Backend executable doesn't have execute permissions. Attempting to fix...`
    );
    if (os.platform() !== "win32") {
  ...
  if (await checkBackendReachable(`http://localhost:${BACKEND_PORT}/health`)) {
    console.log(
      "Backend appears to be running already, but we just killed port processes. This should not happen."
    );
  ...
  try {
    ...
    if (os.platform() === "win32") {
      console.log(
        `Starting Windows backend with: "${backendPath}" in directory: ${backendDirectory}`
      );
    }
    ...
    console.log(
      "Backend process started, waiting for it to become reachable..."
    );
...
async function waitForBackend() {
  ...
  dialog.showErrorBox(
    "Backend Error",
    "The backend service failed to start in time."
  );
  return false;
}
...
function terminateBackend() {
  return new Promise(async (resolve) => {
    if (backendProcess) {
      console.log(
        `Terminating backend process with PID: ${backendProcess.pid}`
      );
      ...
      if (os.platform() === "win32") {
        exec(`taskkill /pid ${backendProcess.pid} /T /F`, (error) => {
          if (error)
            console.error(`Failed to kill Windows process: ${error.message}`);
          backendProcess = null;
